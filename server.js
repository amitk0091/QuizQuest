...
dotenv.config(); // Make sure you've loaded dotenv here
...
const authMiddleware = async function (req, res, next) {
    ...
    try {
        ...
        const result = await collection.find({ email: req.body.email, password: req.body.password, role : req.body.role }).toArray(); // Convert the cursor to an array
...
app.post('/send_otp', (req, res) => {
    ...
    personData = req.body;
    const generatePassword = function () { // Remove the 'await' here and make it a regular function
        let digits = '0123456789';
        st = "";
        for (let i = 0; i < 6; i++)
            st += digits[Math.floor(Math.random() * 10)];
    }
    ...
    transporter.sendMail(options, (error, info) => { // Correct the method name to 'sendMail'
        if (error) {
    ...
    if (otp == st) {
        ...
        client.connect()
            .then(() => {
                ...
                collection.insertOne(personData)
                    .then((result) => {
